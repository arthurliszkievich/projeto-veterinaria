...
class ConsultaAPITests(AuthenticatedAPITestCase):
    def setUp(self):
        ...
        self.doenca_nao_relacionada_api = DoencaFactory(
            nome="Outra Doença API Teste")
    ...
    def test_listar_consultas(self):
        ...
        ConsultaFactory(paciente=self.paciente_rex,
    ...
    def test_criar_consulta_com_todos_os_campos_texto_anamnese_exame(self):
        ...
        doenca_esperada_sugerida.sintomas_associados.set(
            [sint_api_1, sint_api_2])

        # (Opcional) Você pode criar outra doença não relacionada para garantir que apenas a correta é sugerida
        _ = DoencaFactory(nome="Doença Não Relacionada Neste Teste", sintomas_associados=[
                          SintomaFactory(nome="Outro Sintoma Avulso")])
        ...
        self.assertEqual(response.status_code,
                         status.HTTP_201_CREATED, f"Falha ao criar consulta: {response.data}")
        ...
        self.assertEqual(nova_consulta.anamnese_sistema_respiratorio,
        ...
        self.assertEqual(nova_consulta.diagnosticos_suspeitos.count(),
                         1,
        ...
        self.assertIn(doenca_esperada_sugerida, nova_consulta.diagnosticos_suspeitos.all(),
                      "A doença esperada não está entre os diagnósticos suspeitos da consulta.")

        # Verificar na resposta da API
        diagnosticos_suspeitos_resp = response.data.get(
            'diagnosticos_suspeitos', [])
        self.assertIsNotNone(diagnosticos_suspeitos_resp,
                             "O campo 'diagnosticos_suspeitos' não deve ser None na resposta.")
        self.assertEqual(len(diagnosticos_suspeitos_resp), 1,
        ...
        if diagnosticos_suspeitos_resp:  # Verifica se a lista não está vazia antes de tentar acessar o índice
            ids_sugeridos_resp = [d['id'] for d in diagnosticos_suspeitos_resp]
            self.assertIn(doenca_esperada_sugerida.pk, ids_sugeridos_resp,
    ...
    def test_patch_consulta_removendo_todos_sintomas_limpa_diagnosticos_suspeitos_api(self):
        ...
        diagnosticos_suspeitos_resp = response.data.get(
            'diagnosticos_suspeitos', [])
        self.assertEqual(len(diagnosticos_suspeitos_resp), 0,
